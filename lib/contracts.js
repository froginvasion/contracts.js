...
(function() {
  ...
  */
  ...
  root["import"] = function(exportedObj, moduleName) {
    var applyCheck, getOriginal, k, name, o, orig, res, v, value;
    res = {};
    applyCheck = function(orig) {
      var mod, originalContract, originalValue;
      if (typeof orig.server === "string") {
        mod = new ModuleName(orig.server, "", true);
      } else {
        mod = orig.server;
      ...
      originalValue = orig.originalValue, originalContract = orig.originalContract;
      return originalContract.check(originalValue, mod, moduleName, []);
    };
    getOriginal = function(value) {
      if ((value !== null) && typeof value === "object" || typeof value === "function") {
        return contract_orig_map.get(value);
      }
    };
    for (name in exportedObj) {
      if (!__hasProp.call(exportedObj, name)) continue;
      value = exportedObj[name];
