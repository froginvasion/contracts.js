...
(function() {
  ...
  */
  ...
  overload_fun = function(contractParents, cb) {
    ...
    blameOrThrow = function(fns, pos, neg, errors, parents) {
      if (fns.length === 0) {
        if (contractParents != null) {
          return parent();
        } else {
          return blameM(pos, neg, "Overloaded contract failed!" + errors, parents);
        }
      }
    };
    c = new Contract(cname, "overloaded_fun", function(f, pos, neg, parentKs, stack) {
      var handler, localfuns, p, parents;
      localfuns = funs.slice(0);
      parents = parentKs.slice(0);
      handler = {};
      handler["apply"] = function(target, thisArg, args) {
        var callres, current_arg, delayed_ks, domcontract, domcontracts, e, errors, idx, key, max_i, newK, new_arguments, p, res, rngcontract, rngres, _j, _k, _len1, _len2, _ref;
        localfuns = funs.slice(0);
        errors = [];
        new_arguments = [];
        cb = function(k) {
          ...
          localfuns.filter(function(e) {
            return e;
          });
          return blameOrThrow(localfuns, pos, neg, errors, parents);
        };
        i = 0;
        max_i = args.length;
        while (i < max_i) {
        ...
        if (localfuns.length === 0) {
          blameM(pos, neg, "Overloaded contract failed! (dom)" + errors, parents);
        }
        res = target.apply(thisArg, new_arguments);
        callres = [];
        _ref = getCallrng.call(localfuns);
        for (key = _k = 0, _len2 = _ref.length; _k < _len2; key = ++_k) {
          rngcontract = _ref[key];
          try {
