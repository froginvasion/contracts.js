...
(function() {
  ...
  */
  ...
  var Arr, Coffer, Contract, Fun, ModuleName, Unproxy, Utils, and_, any, arr, blame, blameM, check, checkOptions, contract_orig_map, ctor, ctorSafe, enabled, extend, findCallsite, fun, generic_object, generic_var, getModName, guard, idHandler, infer_primitive, none, not_, object, opt, or_, overload_fun, root, self, show_parent_contracts, unproxy, _RegExp, ___, _blame,
  ...
  fun = function(dom, rng, options) {
    ...
    c = new Contract(contractName, "fun", function(f, pos, neg, parentKs, stack) {
      ...
      */
      ...
      makeHandler = function(dom, rng, options) {
        var functionHandler;
        return functionHandler = function(target, thisArg, args) {
          var bf, boundArgs, checked, checked_args, clean_rng, dep_args, e, i, isrest, max_i, res, thisc, _ref, _ref1;
          ...
          if ((options.rest != null) && !options.rest instanceof Contract) {
            blame(neg, pos, "The rest contract that was supplied isn't a contract");
          }
          if (((_ref = options.rest) != null ? _ref.ctype : void 0) === !"opt") {
            options.rest = root.opt(options.rest);
          }
          i = 0;
          max_i = Math.max(dom != null ? dom.length : void 0, args.length);
          if (typeof (dom != null ? dom.length : void 0) === 'number') {
            if (args.length > dom.length && !options.rest) {
          ...
          while (i < max_i) {
            if (isrest) {
              ...
              if (((_ref1 = dom[i]) != null ? _ref1.ctype : void 0) === "opt" && args[i] !== void 0 && (options.rest != null)) {
                try {
              ...
              } else if (!dom[i] && (options.rest != null)) {
                isrest = true;
                checked = options.rest.check(args[i], neg, pos, parents, stack);
              } else {
                checked = dom[i] ? dom[i].check(args[i], neg, pos, parents, stack) : args[i];
              }
            }
            if (i < args.length) {
          ...
          if (typeof rng === "function") {
            dep_args = Array.isArray(checked_args) ? checked_args : [checked_args];
    ...
    c.instantiate = function() {
      var args, dom_contract, i, j;
      args = [].slice.call(arguments);
      i = 0;
      j = 0;
      while (i < dom.length) {
        dom_contract = dom[i];
        if (typeof dom_contract.instantiate === "function" && j < args.length) {
          dom_contract.instantiate(args[j]);
          j++;
        }
        i++;
      }
      if (typeof rng.instantiate === "function" && j < args.length) {
        return rng.instantiate(args[j]);
      }
    };
    c.calldom = calldom;
    c.callrng = callrng;
  ...
  overload_fun = function(contractParents, blameparents) {
    var args, blameOrThrow, c, cname, f, funs, getCalldom, getCallrng, getObjectContracts, i, isCheck, isDelayedContract, isFun, isObject, _i, _len;
    ...
    if (typeof blameparents === "function") {
      args = Array.prototype.slice.call(args, 1, args.length);
    } else {
      blameparents = null;
    }
    ...
    isCheck = function(k) {
      ...
      if (k.ctype === "opt" && k.k.ctype === "check") {
        return true;
      }
      return false;
    };
    isObject = function(k) {
      ...
      if (k.ctype === "opt" && k.k.ctype === "object") {
        return true;
      }
      return false;
    };
    isDelayedContract = function(k) {
      if (isFun(k) || isObject(k)) {
        return true;
      }
      return false;
    };
    while (i < args.length) {
      c = args[i];
      if (!isDelayedContract(c)) {
        throw new Error("" + c + " is not a function contract");
      }
      funs.push(args[i]);
      i++;
    }
    ...
    blameOrThrow = function(fns, k, c, pos, neg, errors, parents) {
      if (fns.length === 0) {
        if (contractParents != null) {
          return cb(k);
        } else {
          return blame(pos, neg, c, c.cname, parents);
        }
      }
    };
    c = new Contract(cname, "overloaded_fun", function(f, pos, neg, parentKs, stack) {
      var e, handler, localfuns, makeHandler, p, parents, proto;
      ...
      makeHandler = function(nondirect) {
        return function(target, thisArg, args) {
          var cb, current_arg, delayed_ks, delayed_rng, domcontract, domcontracts, e, errors, func, idx, k, max_i, newK, new_arguments, p, res, rngcontract, rngcontracts, _j, _k, _l, _len1, _len2, _len3, _ref;
          if (nondirect) {
            args = Array.prototype.slice.call(arguments);
            target = f;
            thisArg = this;
          ...
          cb = function(k) {
            ...
            return blameOrThrow(localfuns, k, c, pos, neg, errors, parents);
          };
          i = 0;
          max_i = Math.max(args.length, Math.max.apply(localfuns.map(function(f) {
            return f.calldom.length;
          })));
          for (k = _j = 0, _len1 = localfuns.length; _j < _len1; k = ++_j) {
            func = localfuns[k];
            if (args.length > ((_ref = func.calldom) != null ? _ref.length : void 0)) {
              delete localfuns[k];
            }
          }
          localfuns = localfuns.filter(function(e) {
            return e;
          });
          while (i < max_i) {
            current_arg = args[i];
            ...
            for (_k = 0, _len2 = domcontracts.length; _k < _len2; _k++) {
              domcontract = domcontracts[_k];
              try {
                if (isDelayedContract(domcontract.contract && typeof current_arg !== "undefined")) {
                ...
                if (contractParents != null) {
                  p = contractParents.filter(function(e) {
                    return e.contract === domcontract.parent;
                  });
                  if (p.length !== 0) {
                    blameparents(p[0]);
                  }
          ...
          if (localfuns.length === 0 && (contractParents == null)) {
          ...
          for (_l = 0, _len3 = rngcontracts.length; _l < _len3; _l++) {
            rngcontract = rngcontracts[_l];
            try {
              ...
              if (contractParents != null) {
                p = contractParents.filter(function(e) {
                  return e.contract === rngcontract.parent;
                });
                if (p.length !== 0) {
                  blameparents(p[0]);
                }
      ...
      handler["apply"] = function(target, thisArg, args) {
        return makeHandler(false)(target, thisArg, args);
      };
      handler["construct"] = function(target, args) {
        return makeHandler(false)(target, args);
      };
      handler["get"] = function(receiver, name) {
        var e, key, oc, ocproxy, ocs, val, _j, _len1;
        localfuns = funs.slice(0);
        ocs = getObjectContracts.call(localfuns);
        if (ocs.length === 0) {
          return f && f[name];
        }
        for (key = _j = 0, _len1 = ocs.length; _j < _len1; key = ++_j) {
          oc = ocs[key];
          try {
      ...
      handler["set"] = function(receiver, name, val) {
        var e, key, oc, ocproxy, ocs, _j, _len1;
        localfuns = funs.slice(0);
        ocs = getObjectContracts.call(localfuns);
        if (ocs.length === 0) {
          f[name] = val;
      ...
      try {
        ...
        if (typeof f === "function") {
          ...
          proto = f === null ? null : Object.getPrototypeOf(f);
  ...
  extend = function(orig, ext) {
    var c, extendedContract, extendingContract, key, orContract, origContract, val;
    if (!(orig instanceof Contract)) {
      throw new Error("cannot extend non contract");
    }
    if (!(ext instanceof Contract)) {
    ...
    for (key in origContract) {
      if (!__hasProp.call(origContract, key)) continue;
      orContract = origContract[key];
      extendedContract = extendingContract[key];
      if (extendedContract === void 0) {
        break;
      }
      if (extendedContract["value"] && extendedContract["value"] instanceof Contract) {
        extendedContract = extendedContract["value"];
      }
      if (orContract["value"] && orContract["value"] instanceof Contract) {
        orContract = orContract["value"];
      }
      if (!extendedContract.equals(orContract)) {
        console.log("WARN: " + key + " does occur in both contracts, but the contract is not equal.");
      }
    }
    for (key in extendingContract) {
      if (!__hasProp.call(extendingContract, key)) continue;
      val = extendingContract[key];
      if (!Object.hasOwnProperty.call(origContract, key)) {
  ...
  generic_var = function(name) {
    var c, handler;
    handler = function(val, pos, neg, parentKs, stack) {
      var inferred_contract;
      if (!c.instantiated && !c.inferred) {
        inferred_contract = infer_primitive(val);
        c.instantiate(k);
      }
      return this.genericContract.check(val, pos, neg, parentKs, stack);
    };
    c = new Contract(name, "parametric", handler);
    c.instantiated = false;
    c.inferred = false;
    c.instantiate = function(k) {
      if (!this.instantiated) {
        this.genericContract = k;
  ...
  generic_object = function() {
    var name, o, objContract, options, rest;
    objContract = arguments[0], options = arguments[1], name = arguments[2], rest = 4 <= arguments.length ? __slice.call(arguments, 3) : [];
    if (options == null) {
      options = {};
    }
    o = object(objContract, options, name);
    if (!options.generic_contracts) {
      return o;
    }
    o.instantiated = false;
    o.instantiate = function() {
      var args, k, v, _i, _len;
      if (o.instantiated) {
        throw new Error("Generic object contract can't be instantiated twice!");
      }
      args = Array.prototype.slice.call(arguments);
      for (k = _i = 0, _len = args.length; _i < _len; k = ++_i) {
        v = args[k];
        if (!v instanceof Contract) {
          throw new Error("" + v + " isnt a contract");
        }
        if (typeof options.generic_contracts[k].instantiate === "function") {
          options.generic_contracts[k].instantiate(v);
        }
      }
      return o.instantiated = true;
    };
    return o;
  };
  ...
  object = function(objContract, options, name) {
    ...
    c = new Contract(objName(objContract), "object", function(obj, pos, neg, parentKs) {
      var contractDesc, e, handler, invariant, newobj, nonObject, objDesc, op, parents, prop, proto, that, value, _ref, _ref1;
      if (typeof obj === "function" && (options["class"] != null) && options["class"] instanceof Contract) {
        newobj = obj.bind({});
        ...
        newobj.prototype = Proxy(Object.create(obj.prototype), {
          "get": function(target, name, receiver) {
            if (options["class"].oc.hasOwnProperty(name)) {
              return options["class"].oc[name].check(target[name], pos, neg, parentKs);
            } else {
              return target[name];
            }
          }
      ...
      if (options["class"] && !options["class"] instanceof Contract) {
        _blame(pos, neg, "class option isn't a contract", parents);
      }
      if (this.oc["prototype"] !== void 0) {
      ...
      handler["deleteProperty"] = function(target, name) {
        ...
        if (options.invariant) {
          ...
          if (!invariant()) {
            blame(neg, pos, "invariant: " + (options.invariant.toString()), target, parents);
          }
        }
        return true;
      };
      handler["get"] = function(target, name, receiver) {
        if (that.oc.hasOwnProperty(name)) {
          return that.oc[name].value.check(target[name], pos, neg, parents);
        } else if ((options.arrayRangeContract && (options.arrayRange !== undefined)) && (parseInt(name, 10) >= options.arrayRange)) {
          return options.arrayRangeContract.check(target[name], pos, neg, parents);
        } else if (options.stringIndexContract && typeof name === "string") {
          return options.stringIndexContract.check(target[name], pos, neg, parents);
        } else if (options.numberIndexContract && (typeof name === "number")) {
          return options.numberIndexContract.check(target[name], pos, neg, parents);
        } else {
          return target[name];
        }
      };
      handler["set"] = function(target, name, val, receiver) {
        if ((options.extensible === false) && Object.getOwnPropertyDescriptor(target, name) === void 0) {
        ...
        } else if (options.numberIndexContact && (typeof name === "number")) {
  ...
  */
  ...
  Arr = function(c) {
    var k;
    k = new Contract("Array<" + c.toString() + ">", "array", function(val, pos, neg, parents) {
      var parentKs, that, v;
  ...
  root.exports = function(moduleName, original) {
    ...
    return Proxy(original, handler);
  };

  root.setExported = function(exportObj, moduleName) {
    var name, replaceOrig, value;
    replaceOrig = function(value) {
      var orig, originalContract, originalValue;
      orig = null;
      if ((value !== null) && typeof value === "object" || typeof value === "function") {
        orig = contract_orig_map.get(value);
      ...
      if (orig != null) {
        originalValue = orig.originalValue, originalContract = orig.originalContract;
        return contract_orig_map.set(value, {
          originalValue: originalValue,
          originalContract: originalContract,
          server: moduleName
        });
  ...
  root.use = function(exportObj, moduleName) {
    var getCorrectModuleName, getOrig, mod, name, orig, res, value;
    res = {};
    getCorrectModuleName = function(orig) {
      var mod;
